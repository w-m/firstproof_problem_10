...
Equivalently, define $\mathcal{L}(X)\equiv S^\top\vecop(KXZ^\top)$ so that $A=\mathcal{L}^\top\mathcal{L}+\lambda(I\otimes K)$; its adjoint is $\mathcal{L}^\top u=\vecop(KUZ)$ where $U$ is the sparse matrix with $\vecop(U)=Su$, which is implemented by the same scatter/accumulate loop.
...
\]
Cost per application: $O(n^2r+nr^2)$ after one-time setup $O(n^3+r^3)$. In matrix form, $A_0\vecop(X)=\vecop\bigl(K(\alpha KXG+\lambda X)\bigr)$, so applying $A_0^{-1}$ amounts to solving the Sylvester-type equation $\alpha KXG+\lambda X=K^{-1}R$.
