...
def apply_A0inv_vec(x, K, G, lam, alpha=1.0):
    # A0 = alpha (G ⊗ (K@K)) + lam (I ⊗ K), K SPD symmetric, G SPD symmetric.
    n = K.shape[0]
    r = G.shape[0]
    ...
    # divide elementwise by (alpha * sigma_a * lambda_b^2 + lam * lambda_b)
...
def main():
    ...
    alpha = 0.37
    A0 = alpha * np.kron(G, K @ K) + lam * np.kron(np.eye(r), K)

    x = rng.standard_normal(n * r)
    y1 = apply_A0inv_vec(x, K, G, lam, alpha=alpha)
